...
public class RaspRC522 {
 ...
 /* 0000 0011*/
    ...
    /* back_data-最大Length=16;,  back_data-データを返す, back_bits-戻り値のビット数, backLen-バイトを返す */
    ...
    /* Anti-collision detection.,Returns tuple of (error state, tag ID).,back_data-5バイト4バイトtagid + 1バイトチェック */
    ...
    /* データの誤りを検知 , CRC値は、data[]の最後の2バイト */
    private void Calculate_CRC(byte[] data) {
        int i, n;
        /* 割り込みレジスタ 0000 0100 */
        ClearBitMask(DivIrqReg, (byte) 0x04);
        /* FIFOに格納されているバイト数を示す。  1000 0000 FIFOカウンタクリア */
        SetBitMask(FIFOLevelReg, (byte) 0x80);
    ...
    //指定されたブロックアドレスを使用するように認証します。
    //authの前にselect_tag（uid）を使用してタグを選択する必要があります。
    //認証モード       : RFID.auth_a or RFID.auth_b
    //ブロックアドレス : 認証に使用
    //key              : list or tuple(配列) with six bytes key
